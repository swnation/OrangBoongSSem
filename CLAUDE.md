...
22. **저장은 반드시 클라우드(Google Drive) 우선** — 설정, 커스텀 항목, 캐시, 학습 매핑, ntfy 토픽 등 **모든 영속 데이터**는 Drive 마스터에 저장. `var`/런타임 변수에만 저장하는 것은 금지 (UI 임시 상태 제외). localStorage는 오프라인 폴백 전용. 새 기능 추가 시 "이 데이터가 새로고침/기기 변경 후에도 유지되는가?" 반드시 확인
...
39. **작업 완료 시 전체 시스템 통합 검증 필수** — 메인앱(index.html) · 두통 빠른 기록(quick/) · 데일리체크(bung/) 간 연결 누락, 데이터 포맷 불일치, 동기화 누락이 없는지 확인. 구체적으로: ①스크립트 로딩 순서 ②함수 존재 여부(typeof 방어) ③도메인 간 sync 포맷 일치 ④ntfy 액션/URL 해시 일관성 ⑤영양제/운동/약물 키 목록 일치 ⑥SW precache 파일 누락 ⑦다크모드 호환성
