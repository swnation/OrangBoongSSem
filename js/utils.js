...
function showConfirmModal(title,body,buttons) {
  ...
  document.getElementById('confirm-foot').onclick = function(e) {
    ...
    if(btn) { const idx = parseInt(btn.dataset.confirmIdx); const action = _confirmActions[idx]; if(action) { try { new Function(action)(); } catch(err) { console.error('Confirm action error:', err); } } }
