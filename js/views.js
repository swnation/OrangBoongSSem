...
function renderView(view) {
  ...
  else if(view==='meds') { try{area.innerHTML=renderMedsView();}catch(e){area.innerHTML=`<div style="color:red;padding:20px">❌ 투약 오류: ${e.message}<br><pre style="font-size:.7rem;overflow:auto">${e.stack}</pre></div>`;} loadAllUserDomains().then(()=>{if(S.currentView==='meds'){try{area.innerHTML=renderMedsView();}catch(e){area.innerHTML=`<div style="color:red;padding:20px">❌ 투약 오류: ${e.message}<br><pre style="font-size:.7rem;overflow:auto">${e.stack}</pre></div>`;}} }); }
