...
### 🔥 건강관리 도메인 UX 대폭 개선 (우선)

#### 증상 기록 통합 폼 (최우선)
- [ ] **모든 도메인을 하나의 증상 기록 폼에 통합** — 서브섹션으로 도메인 분리
  - 현재 도메인 = 펼침, 나머지 = 접힘 (accordeon)
  - 🧠 마음관리: 기분 선택 + 컨디션 체크(수면/집중/발화/기억/에너지/식욕 1-5) + 증상 칩 + 약물 체크
  - 💪 건강관리: 증상 칩 + 약물 체크(통풍/알레르기 등)
  - 🍼 임신준비: 관계/컨디션 기록
  - 🏃 데일리체크(공통): 영양제 + 운동(DB+강도+단위) + 체중 + 음주
  - 📝 메모
  - 💾 저장 → 각 도메인 logData에 분배 저장
- [ ] 저장 시 해당 도메인 logData에만 관련 항목 분배 (cross-domain write)
- [ ] 도메인 간 데이터 참조 통일 (규칙 #31, #37)

#### 통계 대시보드 재설계
- [ ] **통계 대시보드 재설계** — 편두통 전용 통계를 건강관리용으로 전환. 현재 NRS 추세/증상빈도만 있고 의미 없음
  - 체중 추세 차트, 운동 일수/강도 추세, 영양제 순응도 추세, 약물 순응도 세부 분석
  - 수면/에너지/식욕 등 dailyChecks 기반 컨디션 추세
  - AI 건강 인사이트 (검진결과 + 약물 + 운동 + 체중 종합 분석)
- [ ] **캘린더 상세 확장** — bung/ 데일리체크 수준으로 (운동 DB 직접 입력 포함, 현재 요약만 표시)
- [ ] **타임라인 의미화** — 단순 기록 배열 → 약물 변경점/검진 결과/컨디션 변화/운동 패턴 등 이벤트 통합 타임라인
- [ ] 통계/캘린더/타임라인 변경 시 메인앱↔bung/ 동기화 확인 (규칙 #31)
